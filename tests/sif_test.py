...
def test_pure_mode_1():
    ...
    assert np.isclose(K_I, K_I_analytical, rtol=0.005)
...
def test_pure_mode_1_3d():
    ...
    model.insert_ndbsplines_crack(scipy_crack, 2 * W / x_elem, snapping_tolerance=0.03)
    ...
    model.solve_static()
    dcm = DCMSIF3D(kappa, G_mod, np.linspace(0.01, 0.03, 5), None)
