...
for i in range(100):
    ...
    x_elem = 100
    y_elem = x_elem
    nodes, elements = tf.gen_rect_Quad4n(2 * W, 2 * W, x_elem, y_elem)
    ...
    # 1. THE SAFETY FILTER:
    # Calculate distances between consecutive points and drop any that are < 1e-8 apart.
    # This mathematically guarantees SciPy will never receive invalid zero-length chords.
    diffs = np.linalg.norm(np.diff(pts_array, axis=0), axis=1)
    valid_mask = np.insert(diffs > 1e-8, 0, True)
    pts_clean = pts_array[valid_mask]

    # 2. THE SPLINE:
    # We must use k=3 now! The breadcrumbs will keep it stiff,
    # and k=3 will keep FITPACK and your Newton solver perfectly happy.
    tck, u = splprep(pts_clean.T, s=1e-5, k=3)
    bspline = BSpline(tck[0], np.transpose(tck[1]), tck[2])
    # pts = np.array(pts_list).T
    # tck, u = splprep(pts, s=1e-5, k=3)
    # bspline = BSpline(tck[0], np.transpose(tck[1]), tck[2])
    h = 2 * W / x_elem
    ...
    fix_dofs.extend(
        list(
            model.list_dof.get_elem_dof_numbers_flat(
                nodes_to_constrain, DofType.UX | DofType.UY
            )
        )
    )
    ...
    if i == 0:
        new_tip = tip + 7 * da_dn * new_direction
    else:
