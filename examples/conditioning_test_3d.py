...
def run_condition_number_study_3d():
    ...
    n_vals = np.array([13, 21, 31, 51, 71, 101])
    h_vals = 10.0 / n_vals
    ...
    for i in n_vals:
        ...
        np.arange(1, len(nodes) + 1)
        np.where(np.abs(nodes[:, 2]) < tol)[0] + 1  # y=0 plane
        ...
        model_sc.gen_dirichlet_bc(sel_condition)
        ...
        model_fem.cal_global_matrices({"Tetr4n": Tetr4n})

        def sel_condition(x, y, z):
            return z - 0.0

        model_fem.gen_dirichlet_bc(sel_condition)
        ...
        if i <= 31:
            ...
            model_std.gen_dirichlet_bc(sel_condition)
