...
from ..core.dofs import BASE_DOFS, BRANCH_DOFS, HEAVISIDE_DOFS
...
def partial_cut_embedding_tetr_iter(Nc, tip, tip_on_interface, range_iter=range(17)):
    ...
    for k in range_iter:
        ...
        if np.isclose(detJik, 0.0, atol=1e-16):
            continue
...
def cut_embedding_tri_iter(Nc, range_iter=range(4)):
    ZERO_TOL = 1e-10
    EYE = np.eye(3)
    # Nc colums:
    #  0  1  2
    ...
    for i in range_iter:
        ...
        if i != 3:
            node_idx, e1, e2 = TRI_SPLIT_MAP[i]
            Ni[:, i] = EYE[:, node_idx]
            Ni[:, (i + 1) % 3] = Nc[:, e1]
            Ni[:, (i + 2) % 3] = Nc[:, e2]
        else:
...
# def partial_cut_embedding_tri_iter(Nc, tip, range_iter):
#     ZERO_TOL = 1e-10
...
def partial_cut_embedding_tri_iter(Nc, tip, range_iter=range(4)):
    ZERO_TOL = 1e-10
    EYE = np.eye(3)
    # Nc colums:
    #  0  1  2
    ...
    for i in range_iter:
        ...
        node_idx, e1 = TRI_SPLIT_MAP[i]
        Ni[:, 1 + i % 2] = EYE[:, node_idx]
        Ni[:, 2 - (i % 2)] = Nc[:, e1]
...
def fill_element_displacement(elem_nodes, list_dof, Ug):
    ...
    Ue = np.zeros(n_nodes * np.bitwise_count(local_dofs_per_node))
    DOFs = np.concatenate(
        (
