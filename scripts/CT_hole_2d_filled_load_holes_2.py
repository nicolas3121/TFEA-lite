...
p3 = gmsh.model.occ.addPoint(W - dist_opp_edge, 0, 0)  # TIP OF NOTCH
...
# --- TOPOLOGICAL SLICING ---
# Forces Gmsh to align quad elements horizontally ahead of the tip.
p_slice_end = gmsh.model.occ.addPoint(W, 0, 0)
slice_line = gmsh.model.occ.addLine(p3, p_slice_end)

# --- BOOLEAN OPERATIONS ---
# Step A: Cut OUT the main hole and V-notch
cut_out, _ = gmsh.model.occ.cut([(2, plate)], [(2, main_hole), (2, notch)])
plate_tag = cut_out[0][1]

# Step B: FRAGMENT the plate with the pins AND the slicing line
gmsh.model.occ.fragment([(2, plate_tag)], [(2, top_pin), (2, bot_pin), (1, slice_line)])
...
for dim, tag in surfaces:
    ...
    if abs(com[0] - load_hole_x) < 1e-3 and abs(com[1] - load_hole_y) < 1e-3:
        ...
        gmsh.model.mesh.embed(0, [pt_top], 2, tag)  # Point securely embedded
    elif abs(com[0] - load_hole_x) < 1e-3 and abs(com[1] - (-load_hole_y)) < 1e-3:
        pin_surfs.append(tag)
        gmsh.model.mesh.embed(0, [pt_bot], 2, tag)  # Point securely embedded
    else:
...
gmsh.option.setNumber("Mesh.RecombinationAlgorithm", 3)  # Blossom-Quad

# -> SubdivisionAlgorithm DELIBERATELY REMOVED to allow safe triangles at the pins <-

# 5. Define the Dense Crack Path (Box Field)
gmsh.model.mesh.field.add("Box", 1)

# Base sizes restored to 0.2 because we are no longer splitting elements!
